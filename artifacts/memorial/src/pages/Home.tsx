...
const PERSONAS_HOMENAJE: { nombre: string; foto: string | null }[] = [
  { nombre: "Ana Soledad Lizarazo Calderón",   foto: "/foto-ana.jpg" },
  { nombre: "Pablo Esteban Aguirre Camargo",   foto: "/foto-pablo.jpg" },
  { nombre: "Carlos Alberto Camargo Munevar",  foto: "/foto-carlos.jpg" },
];
  ...
  return (
    <section style={{ background: ESPRESSO, borderBottom: `1px solid rgba(201,148,58,0.18)` }}>
      <div className="max-w-5xl mx-auto flex flex-col items-center px-6" style={{ paddingTop: "4.5rem", paddingBottom: "4rem" }}>
        ...
        <div className="sm:hidden w-full flex flex-col items-center"><HeroCarousel /></div>
        <div className="hidden sm:flex flex-row items-start justify-center gap-8 sm:gap-14 w-full">
          {PERSONAS_HOMENAJE.map(({ nombre, foto }, i) => <PersonaCard key={nombre} nombre={nombre} foto={foto} delay={i * 0.15} />)}
        </div>
    ...
    <div className="min-h-screen" style={{ background: CREAM, color: ESPRESSO }}>
      ...
      <section className="py-24 px-4 text-center" style={{ background: ESPRESSO }}>
        ...
        <div className="flex flex-col sm:flex-row items-center gap-6 justify-center">
          ...
          <Link href="/recuerdos" className="text-xs uppercase tracking-[0.28em] transition-opacity hover:opacity-70" style={{ color: GOLD }}>— Ver recuerdos</Link>
        </div>
      </section>
